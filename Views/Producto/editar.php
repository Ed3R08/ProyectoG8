...
<body>
    ...
    <?php if ($producto): ?>
        ...
        <form action="../../Controllers/productoController.php?action=editar" method="post" class="mt-4" enctype="multipart/form-data">
            <input type="hidden" name="id" value="<?= htmlspecialchars($producto['id_producto']) ?>">

            <label>Categoría</label>
            <select name="idCategoria" class="form-control" required>
                <?php foreach ($categorias as $cat): ?>
                    <option value="<?= $cat['id_categoria'] ?>" <?= (isset($producto['id_categoria']) && $cat['id_categoria'] == $producto['id_categoria']) ? 'selected' : '' ?>>
            ...
            <input type="number" name="existencias" class="form-control"
                   value="<?= htmlspecialchars($producto['existencias']) ?>" required>

            <label>Ruta actual de la imagen</label>
            ...
            <button name="btnEditarProducto" type="submit" class="btn btn-success mt-3">Guardar</button>
        </form>
